  ...
  <div>
    ...
    <div class="px-3 py-2">
      <b-row class="mb-2 style-menu" :class="scY > 0 ? 'top-menu' : ''" :style="isAdsHidden ? 'top: 128px;' : 'top: 254px;'" v-if="iframe != '1'">
        <b-col cols="3">
          <center2-title type="muaythai" :live="path == 'live' ? true : false" :path="path"></center2-title>
        </b-col>
      ...
      if (this.liveData.data.data.length == 0) {
        ...
        if (localStorage.getItem('filterSport')) {
          ...
          for (const data in array) {
            ...
            // const kickoffdate   = moment(new Date(array_data[0].real_kickoff_time)).format("YYYY-MM-DD") > moment(new Date(array_data[array_data.length-1].real_kickoff_time)).format("YYYY-MM-DD") ? array_data[0].real_kickoff_time :array_data[array_data.length-1].real_kickoff_time
            // array_data[0].newLeague = true
            // for (let i = 0; i < array_data.length; i++) {
            //       array_data[i].kickoffdate = kickoffdate
            //       array2.push(array_data[i])
            // }
          }

          array2.sort((a, b) => {
            let an = a.sortleague //'000-'+a.kickoffdate+'-3-'+a.tournament[0]// '000-'+a.tournament[0]
            let bn = b.sortleague //'000-'+b.kickoffdate+'-3-'+b.tournament[0]// '000-'+b.tournament[0]
            if (an < bn) {
              return -1;
            }
            if (an > bn) {
      ...
      if (this.nonLiveData.data.data.length == 0) {
        ...
        if (localStorage.getItem('filterSport')) {
          ...
          for (const data in array) {
            ...
            // const kickoffdate   = moment(new Date(array_data[array_data.length-1].real_kickoff_time)).format("YYYY-MM-DD") > moment(new Date(array_data[0].real_kickoff_time)).format("YYYY-MM-DD") ? array_data[array_data.length-1].real_kickoff_time : array_data[0].real_kickoff_time//array_data[array_data.length-1].real_kickoff_time
            //   array[data][0].newLeague = true
            //   for (let i = 0; i < array_data.length; i++) {
            //     array_data[i].kickoffdate = kickoffdate
            //     array2.push(array_data[i])
          ...
          for (const data in array) {
            // let data1 = array[data].sort((a, b) => b.real_kickoff_time.localeCompare(a.real_kickoff_time))
            // console.log("data_kicofftimedata_kicofftime",data1);
            // let array_data      = array[data] //.sort((a, b) => b.real_kickoff_time.localeCompare(a.real_kickoff_time))
            // const kickoffdate   = moment(new Date(array_data[array_data.length-1].real_kickoff_time)).format("YYYY-MM-DD") > moment(new Date(array_data[0].real_kickoff_time)).format("YYYY-MM-DD") ? array_data[array_data.length-1].real_kickoff_time :array_data[0].real_kickoff_time //array_data[array_data.length-1].real_kickoff_time
            // array_data[0].newLeague = true
            // for (let i = 0; i < array_data.length; i++) {
            //   array_data[i].kickoffdate = kickoffdate
  ...
  methods: {
    ...
    async fetchgetnonLive(type, start) {
      ...
      if (this.oldPriceNonLive == null) {
        ...
        if (this.oldPriceNonLive != price) {
          ...
          var nonlive = await this.$axios.get(
            ...
            {
              params: { 'sport_type_id': '44', 'odds_type': type != 'sc' ? 'EU' : price }
            }
          );
      ...
      if (start == 'start') {
        ...
        if (this.data_change_id_non) {
          ...
          let response = await this.$axios.get("event/" + route_path + "/nokey/get-nonlive-odds_nonfullload?sport_type_id=44&odds_type=" + price + "&change_id=" + this.data_change_id_non);
    ...
    async fetchgetLive(type, start) {
      ...
      if (start == 'start') {
        ...
        if (this.data_change_id) {
          ...
          let response = await this.$axios.get("event/" + route_path + "/nokey/get-live-odds_nonfullload?sport_type_id=44&odds_type=" + price + "&change_id=" + this.data_change_id);
    ...
    async fetchgetEarly(type) {
      ...
      var earlydata = await this.$axios.get(
        ...
        {
          params: { 'sport_type_id': '44', 'odds_type': type != 'sc' ? 'EU' : price, 'date': date, }
        }
      );
