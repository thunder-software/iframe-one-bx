  ...
  <div>
    ...
    <div class="px-3 py-2">
      <b-row class="mb-2 style-menu" :class="scY > 0 ? 'top-menu' : ''" :style="isAdsHidden ? 'top: 128px;' : 'top: 254px;'" v-if="iframe != '1'">
        <b-col cols="3">
          <div is="center2-title" type="basketball" :live="path == 'live' ? true : false" :path="path"></div>
        </b-col>
        <b-col cols="9" class="text-right pr-0">
          <div is="center2-select-league-button" :displaytable="getselectleague" @filterlive="filterlive($event)"
            :loading="loading" @refresh="fetchGetData" @filternonlive="filternonlive($event)" :oldlive="oldlive"
            :oldnotlive="oldnotlive" :getlive="favoriteGetLive" :getnotlive="favoriteNonLive" :getearly="favoriteEarly">
          </div>
        </b-col>
      </b-row>
      ...
      <b-row class="style-menu-row" :style="!isAdsHidden ? 'margin-top: 190px;' : ''">
        <b-col cols="12">
          <div is="center2-match-table-live" v-if="path != 'early'" status="live" @MoreBet="MoreBet($event, 'live')"
            :setData="favoriteGetLive" :displaytable="getselectleague" :loading="loading" @fetch="fetchGetData"
            class="mb-3"></div>
          <div is="center2-match-table-nonlive" v-if="path != 'early' && path != 'live'" status="nonLive"
            @MoreBet="MoreBet($event, 'nonLive')" :setData="favoriteNonLive" :displaytable="getselectleague"
            :loading="loading" @fetch="fetchGetData"></div>
  ...
  computed: {
    ...
    setDataEarly() {
      ...
      if (this.earlyData.data.data.length == 0) {
        ...
        array = this.sortData(this.mapData(this.earlyData)).reduce((group, match) => {
          const sport_type_id = match.tournament.tournament_id;
          group[sport_type_id] = group[sport_type_id] ?? [];
          group[sport_type_id].push(match);
          return group;
    ...
    async fetchgetnonLive(type, start) {
      ...
      if (this.oldPriceNonLive == null) {
        ...
        if (this.oldPriceNonLive != price) {
          ...
          var nonlivedata = await this.$axios.get(
            ...
            {
              params: { 'sport_type_id': '2', 'odds_type': type != 'sc' ? 'EU' : price, nav: this.navSide }
            }
          );
      ...
      if (start == 'start') {
        ...
        if (this.data_change_id_non) {
          ...
          let response = await this.$axios.get("event/" + route_path + "/nokey/get-nonlive-odds_nonfullload?sport_type_id=2&odds_type=" + price + "&change_id=" + this.data_change_id_non);
