  ...
  <div>
    ...
    <div class="px-3 py-2">
      <b-row class="mb-2 style-menu" :class="scY > 0 ? 'top-menu' : ''" :style="isAdsHidden ? 'top: 128px;' : 'top: 254px;'" v-if="iframe != '1'">
        <b-col cols="3">
          <center2-title type="snooker" :live="path == 'live' ? true : false" :path="path"></center2-title>
        </b-col>
        <b-col cols="9" class="text-right pr-0">
          <center2-select-league-button :displaytable="getselectleague" @filterlive="filterlive($event)"
            ...
            :oldnotlive="oldnotlive" :getlive="favoriteGetLive" :getnotlive="favoriteNonLive"
            :getearly="favoriteEarly"></center2-select-league-button>
        </b-col>
      </b-row>
    ...
    async fetchgetnonLive(type, start) {
      ...
      if (this.oldPriceNonLive == null) {
        ...
        if (this.oldPriceNonLive != price) {
          ...
          var nonlivedata = await this.$axios.get("/event/" + type + "/nokey/get-nonlive-odds",
            {
              params: { 'sport_type_id': '6', 'odds_type': type != 'sc' ? 'EU' : price, nav: this.navSide }
            }
          );
      ...
      if (start == 'start') {
        ...
        if (this.data_change_id_non) {
          ...
          let response = await this.$axios.get("event/" + route_path + "/nokey/get-nonlive-odds_nonfullload?sport_type_id=6&odds_type=" + price + "&change_id=" + this.data_change_id_non);
    ...
    async fetchgetLive(type, start) {
      ...
      if (start == 'start') {
        ...
        if (this.data_change_id) {
          ...
          let response = await this.$axios.get("event/" + route_path + "/nokey/get-live-odds_nonfullload?sport_type_id=6&odds_type=" + price + "&change_id=" + this.data_change_id);
