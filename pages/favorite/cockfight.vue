  ...
  <div>
    ...
    <div class="px-3 py-2">
      <b-row class="mb-2 style-menu" :class="scY > 0 ? 'top-menu' : ''" :style="isAdsHidden ? 'top: 128px;' : 'top: 254px;'" v-if="iframe != '1'">
        <b-col cols="3">
          <center2-title type="cockfight" :live="path == 'live' ? true : false" :path="path"></center2-title>
        </b-col>
  ...
  methods: {
    ...
    async fetchgetnonLive(type, start) {
      ...
      if (this.oldPriceNonLive == null) {
        ...
        if (this.oldPriceNonLive != price) {
          ...
          var nonlive = await this.$axios.get("/event/" + type + "/nokey/get-nonlive-odds",
            {
              params: { 'sport_type_id': '35', 'odds_type': type != 'sc' ? 'EU' : price }
            }
          );
      ...
      if (start == 'start') {
        ...
        if (this.data_change_id_non) {
          ...
          let response = await this.$axios.get("event/" + route_path + "/nokey/get-nonlive-odds_nonfullload?sport_type_id=35&odds_type=" + price + "&change_id=" + this.data_change_id_non);
    ...
    async fetchgetLive(type, start) {
      ...
      if (start == 'start') {
        ...
        if (this.data_change_id) {
          ...
          let response = await this.$axios.get("event/" + route_path + "/nokey/get-live-odds_nonfullload?sport_type_id=35&odds_type=" + price + "&change_id=" + this.data_change_id);
    ...
    async fetchgetEarly(type) {
      ...
      var earlydata = await this.$axios.get(
        ...
        {
          params: { 'sport_type_id': '35', 'odds_type': type != 'sc' ? 'EU' : price, 'date': date, }
        }
      );
