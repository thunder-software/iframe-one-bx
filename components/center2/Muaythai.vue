    ...
    <div class="" style="padding: 8px 8px;">
      ...
      <b-row class="mb-2 style-menu pr-0" :class="{'bet-close-table' : (checkBetAll)}" :style="isAdsHidden ? 'top: 128px;' : 'top: 254px;'" v-if="iframe != '1'">
        ...
        <b-col cols="9" class="text-right pr-1">
          <div is="center2-select-league-button"
      ...
      <div class="class-tv-box" v-if="getDataTV">
        ...
        <b-col cols="12">
          ...
          <!-- <b-img src="/banner/header1.png"></b-img> -->
          <div is="center2-match-table-live-mpy"
            ...
            class="mb-3"
          ></div>
          <div is="center2-match-table-nonlive-mpy"
            ...
            @fetch="fetchGetData"
          ></div>
          <div is="center2-match-table-early-mpy"
            v-if="path == 'early'"
            status="early"
...
export default {
      ...
      if (this.earlyData.data.data.length == 0) {
        return [];
                ...
                var newNumber = '1'
                  ...
                  // array2.sort((a, b) => {
                  //     let valueA = a.sortleague.split('-').map(Number).join('');
                  //     let valueB = b.sortleague.split('-').map(Number).join('');
  ...
  methods: {
    ...
    async fetchgetnonLive(type , start) {
      ...
      if(this.oldPriceNonLive == null){
        ...
        if(this.oldPriceNonLive != price){
          ...
          if(this.$route.params.path != 'step' && this.$route.params.path != 'mixparlay'){
            setData = {'sport_type_id' : '44' , 'odds_type' : type != 'sc' ? 'EU' : price, 'sortTime':getSortTime}
          }else{
            setData = {'sport_type_id' : '44' , 'odds_type' : type != 'sc' ? 'EU' : price, 'sortTime':getSortTime , 'date' : this.date}
          }
      ...
      //       price = 'EU'
      //     }
      //   let response = await this.$axios.get("event/"+route_path+"/nokey/get-nonlive-odds_nonfullload?sport_type_id=44&odds_type="+price+"&change_id=" + this.data_change_id_non+'&sortTime='+getSortTime);
      ...
      //       "/event/"+type+"/nokey/get-nonlive-odds" ,
      //       {
      //         params: {'sport_type_id' : '44' , 'odds_type' : type != 'sc' ? 'EU' : price, 'sortTime':getSortTime}
      //       }
      //     );
      //     this.data_change_id_non  = nonlive.data.change_id
      //     this.nonLiveData = this.newData(nonlive , 'Nosoccer')
    ...
    async fetchgetLive(type  , start) {
      ...
      //       price = 'EU'
      //     }
      //       let response = await this.$axios.get("event/"+route_path+"/nokey/get-live-odds_nonfullload?sport_type_id=44&odds_type="+price+"&change_id=" + this.data_change_id+ '&sortTime='+getSortTime);
     ...
     async fetchgetEarly(type) {
      ...
      var earlydata = await this.$axios.get(
        ...
        {
          params: {'sport_type_id' : '44' , 'odds_type' : type != 'sc' ? 'EU' : price, 'date' : date , 'sortTime':getSortTime}
        }
      );
      ...
      this.earlyData = date == 'Allearly'? arrayDate : newEarly
    ...
    async fetchGetData(type) {
      ...
      this.path = route.substring(10, 99);
      if (this.path == "today") {
