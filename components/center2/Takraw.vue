  ...
  methods: {
    ...
    async fetchgetnonLive(type , start) {
      ...
      if(this.oldPriceNonLive == null){
        ...
        if(this.oldPriceNonLive != price){
          ...
          if(this.$route.params.path != 'step' && this.$route.params.path != 'mixparlay'){
            setData = {'sport_type_id' : '37' , 'odds_type' : type != 'sc' ? 'EU' : price, 'sortTime':getSortTime}
          }else{
            setData = {'sport_type_id' : '37' , 'odds_type' : type != 'sc' ? 'EU' : price, 'sortTime':getSortTime , 'date' : this.date}
          }
      ...
      this.loading = false
        ...
        let response = await this.$axios.get("event/"+route_path+"/nokey/get-nonlive-odds_nonfullload?sport_type_id=37&odds_type="+price+"&change_id=" + this.data_change_id_non+'&sortTime='+getSortTime);
    ...
    async fetchgetLive(type , start) {
      ...
      if(this.data_change_id){
       ...
       let response = await this.$axios.get("event/"+route_path+"/nokey/get-live-odds_nonfullload?sport_type_id=37&odds_type="+price+"&change_id=" + this.data_change_id+'&sortTime='+getSortTime);
     ...
     async fetchgetEarly(type) {
      ...
      var earlydata = await this.$axios.get(
        ...
        {
          params: {'sport_type_id' : '37' , 'odds_type' : type != 'sc' ? 'EU' : price, 'date' : date , 'sortTime':getSortTime}
        }
      );
    ...
    async fetchGetData(type) {
      ...
      this.path = route.substring(8, 99);
      if (this.path == "today") {
