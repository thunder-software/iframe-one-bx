  ...
  methods: {
    ...
    async fetchgetnonLive(type , start) {
      ...
      if(this.oldPriceNonLive == null){
        ...
        if(this.oldPriceNonLive != price){
          this.oldPriceNonLive = price
            ...
            {
              params: {'sport_type_id' : '10035' , 'odds_type' : type != 'sc' ? 'EU' : price, 'sortTime':getSortTime}
            }
          );
      ...
      if(start == 'start'){
        ...
        let response = await this.$axios.get("event/"+route_path+"/nokey/get-nonlive-odds_nonfullload?sport_type_id=10035&odds_type="+price+"&change_id=" + this.data_change_id_non+ '&sortTime='+getSortTime);
    ...
    async fetchgetLive(type , start) {
      ...
      if(start == 'start'){
        ...
        let response = await this.$axios.get("event/"+route_path+"/nokey/get-live-odds_nonfullload?sport_type_id=10035&odds_type="+price+"&change_id=" + this.data_change_id+'&sortTime='+getSortTime);
    ...
    async fetchgetEarly(type) {
      ...
      var earlydata = await this.$axios.get(
        ...
        {
          params: {'sport_type_id' : '10035' , 'odds_type' : type != 'sc' ? 'EU' : price, 'date' : date , 'sortTime':getSortTime}
        }
      );
