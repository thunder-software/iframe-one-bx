<template>
    <tbody :class="fontSize(displaytable.fontSize)">
      <tr v-if="tournament.period != '2' && tournament.period != '5'">
          ...
          <td
            ...
            <!-- 'hidden' : hiddenRow(data.odds , 'FH_Handicap' ,i) == '1' -->
            <tr v-for="(data, i) in getData" :key="i" v-if="data.priceRow != 0 && data.collapse != 'open'" :class="{'odd' :data.count_match % 2 == 0 }" :style="checkStatusTable(data.period)">
      ...
      <tr v-for="(data, i) in getData" :key="i" v-if="data.priceRow == 0 && i == 0 && data.collapse != 'open'" :style="checkStatusTable(data.period)">
        <td class="align-center w-12" colspan="2" style="height: 36px">
              <div is="center2-column-time2"
                :status="status"
                :match="data"
      ...
      getData() {
        ...
        if (result) {
          ...
          const results = result.reduce((row, item) => {    
            if(row){
              if(item.period != '2' || item.period != '5'){              
              ...
              this.tournament.period = result[0].period
              ...
              item.setting_w_b_l.forEach((setting, i) => {
                    ...
                    setting.sort_row = setting.sort_row
                      ...
                      let datas = {
                        ...
                        priceGroups1x2 : item.priceGroups['1x2'],
